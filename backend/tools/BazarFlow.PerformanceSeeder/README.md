...
Current status: `V2-06B-3B`. It validates CLI options, validates database safety rules, prints deterministic dry-run previews, and can insert core synthetic reference data:
...
When `--include-transactions` is passed, it also creates synthetic purchases, purchase lines, product batches, sales invoices, and invoice lines. It does not create inventory allocations, blackbox events, audit logs, or reset data.
...
- Reset is not implemented in `V2-06B-3B`. If `--reset` is passed in non-dry-run mode, the tool refuses to run writes and asks you to remove `--reset`.
...
`--include-transactions` adds purchases, product batches, sales invoices, and invoice lines after the core reference data path:
...
| Profile | Purchases | Purchase Lines | Product Batches | Invoices | Invoice Lines |
| --- | ---: | ---: | ---: | ---: | ---: |
| small | 100 | 500-2,000 | 500-2,000 | 1,000 | 1,000-20,000 |
| medium | 1,000 | 5,000-20,000 | 5,000-20,000 | 10,000 | 10,000-500,000 |
| large | 5,000 | 25,000-100,000 | 25,000-100,000 | 50,000 | 50,000-4,000,000 |

Large profile can take time and should only be run against a dedicated performance database.

Stock allocation and profit-complete reporting are deferred to V2-06B-3C. V2-06B-3B does not create `InvoiceLineBatchAllocation` rows and does not decrement `ProductBatch.QuantityAvailable`.

## Future Phases

- Add synthetic-only reset support with double confirmation.
- Add invoice allocations, blackbox event, and audit log generation where schema constraints allow.
